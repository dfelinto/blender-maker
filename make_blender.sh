...
scripts="${BLENDER}/git/blender/release/scripts"
...
if [ -z $BASE_TARGET ]; then
  ...
  case $myos in
    Darwin)
      BASE_TARGET="${release}/bin/blender.app/Contents/Resources/"
      ;;
    Linux)
...
if [ $UPDATE -eq 1 ]; then
  ...
  # target is to copy seleted scripts files only#
